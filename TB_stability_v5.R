...
#stats
#untransformed
anova_t3(IndVars = c("year", "invasion_percent"), 
         DepVar = "rel_cov", 
...
#graph this
#rel inv % vs stability of bromes
BRARcovstab2 <- ggplot(data = BRARcovstab, aes(x = avgBRAR, y = stability)) + 
  geom_point(size = 3) + 
  #ylim(0, 27) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "BRAR Cover Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRTEcovstab2 <- ggplot(data = BRTEcovstab, aes(x = avgBRTE, y = stability)) + 
  geom_point(size = 3) + 
  #ylim(0, 27) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "BRTE Cover Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRARcovstab2 / BRTEcovstab2


#statistics
BRARcovstab_stats <- lmerTest::lmer(data = BRARcovstab, stability ~ avgBRAR +
                                       (1|gradient_number))
anova(BRARcovstab_stats, type = 3) #p = 0.006411
rsquared(BRARcovstab_stats) #marg = 0.2327043, cond = 0.5161897

BRTEcovstab_stats <- lmerTest::lmer(data = BRTEcovstab, stability ~ avgBRTE +
                                      (1|gradient_number))
anova(BRTEcovstab_stats, type = 3) #p = 0.002806
rsquared(BRTEcovstab_stats) #marg = 0.3159394, cond = 0.3486865


#check normality
#BRAR cov stab
resid_1 <- lm(data = BRARcovstab, stability ~ avgBRAR)
ols_plot_resid_hist(resid_1) #right skew  
ols_test_normality(resid_1) #fail


#try transformation - ln looks good
BRARcovstab_trans <- BRARcovstab %>%
...
resid_2 <- lm(data = BRARcovstab_trans, ln_stab ~ avgBRAR)
ols_plot_resid_hist(resid_2) #better 
ols_test_normality(resid_2) #passed KS

#retry stats
BRARcovstab_stats2 <- lmerTest::lmer(data = BRARcovstab_trans, ln_stab ~ avgBRAR +
                                        (1|gradient_number))
anova(BRARcovstab_stats2, type = 3) #p = 5.402e-5
rsquared(BRARcovstab_stats2) #marg = 0.4202505; cond = 0.694937


#BRTE cov stab
resid_3 <- lm(data = BRTEcovstab, stability ~ avgBRTE)
ols_plot_resid_hist(resid_3) #right skew  
ols_test_normality(resid_3) #pass KS

#try transformation - ln looks good
BRTEcovstab_trans <- BRTEcovstab %>%
  mutate(sqrt_stab = sqrt(stability)) %>%
  mutate(ln_stab = log10(stability + 0.1)) %>%
  mutate(sq_stab = stability^2) %>%
  mutate(cb_stab = stability^1/3) %>%
...
resid_4 <- lm(data = BRTEcovstab_trans, ln_stab ~ avgBRTE)
ols_plot_resid_hist(resid_4) #better 
ols_test_normality(resid_4) #passed KS, SW, AD

#retry stats
BRTEcovstab_stats2 <- lmerTest::lmer(data = BRTEcovstab_trans, ln_stab ~ avgBRTE +
                                       (1|gradient_number))
anova(BRTEcovstab_stats2, type = 3) #p = 8.357e-5
rsquared(BRTEcovstab_stats2) #marg = 0.4925643; cond = 0.5040123
...
BRARrichstab3_nobrome <- ggplot(data = BRARrichstab2_nobrome, aes(x = avgBRAR, y = stability)) + 
  ...
  ylim(0, 27) +
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Richness Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))
...
BRTErichstab3_nobrome <- ggplot(data = BRTErichstab2_nobrome, aes(x = avgBRTE, y = stability)) + 
  ...
  ylim(0, 27) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Richness Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRARrichstab3_nobrome / BRTErichstab3_nobrome
...
BRARevenstab3_nobrome <- ggplot(data = BRARevenstab2_nobrome, aes(x = avgBRAR, y = stability)) + 
  ...
  ylim(0, 15) +
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Evenness Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))
...
BRTEevenstab3_nobrome <- ggplot(data = BRTEevenstab2_nobrome, aes(x = avgBRTE, y = stability)) + 
  ...
  labs(x = "BRTE Rel Cov (%)", y = "Evenness Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRARevenstab3_nobrome / BRTEevenstab3_nobrome
...
#graph - rel cov vs synch
BRARsynch_fig_nobrome <- ggplot(data = BRARsynch_nobrome2, aes(x = avgBRAR, y = synch)) + 
  geom_point(size = 3) + 
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Synchrony", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRTEsynch_fig_nobrome <- ggplot(data = BRTEsynch_nobrome2, aes(x = avgBRTE, y = synch)) + 
  geom_point(size = 3) + 
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Synchrony", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRARsynch_fig_nobrome / BRTEsynch_fig_nobrome
...
#graph - rel cov vs turnover
noBRAR_turn_cov2 <- ggplot(data = noBRAR_turn_cov, aes(x = avgBRAR, y = total)) + 
  geom_point(size = 3) + 
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Species Turnover", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

noBRTE_turn_cov2 <- ggplot(data = noBRTE_turn_cov, aes(x = avgBRTE, y = total)) + 
  geom_point(size = 3) + 
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Species Turnover", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

noBRAR_turn_cov2 / noBRTE_turn_cov2
...
#graph - rel cov vs stability
noBRAR_stab3 <- ggplot(data = BRARcov_only3, aes(x = avgBRAR, y = stability)) + 
  geom_point(size = 3) + 
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Cover Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

noBRTE_stab3 <- ggplot(data = BRTEcov_only3, aes(x = avgBRTE, y = stability)) + 
  geom_point(size = 3) + 
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Cover Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

noBRAR_stab3 / noBRTE_stab3
...
BRAR_barestab2 <- ggplot(data = BRAR_barestab, aes(x = avgBRAR, y = stability)) + 
  ...
  ylim(0, 25) +
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Bareground Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))
...
BRTE_barestab2 <- ggplot(data = BRTE_barestab, aes(x = avgBRTE, y = stability)) + 
  ...
  ylim(0, 10) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Bareground Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRAR_barestab2 / BRTE_barestab2
...
#graphs
BRAR_pctstab2 <- ggplot(data = BRAR_pctstab, aes(x = avgBRAR, y = log10(stability))) + 
  geom_point(size = 3) + 
  #ylim(0, 110) +
  ylim(0, 2.5) +
  stat_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Light Transmittance Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRTE_pctstab2 <- ggplot(data = BRTE_pctstab, aes(x = avgBRTE, y = log10(stability))) + 
  geom_point(size = 3) + 
  #ylim(0, 110) +
  ylim(0, 2.5) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Light Transmittance Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRAR_pctstab2 / BRTE_pctstab2
...
BRAR_smstab2 <- ggplot(data = BRAR_smstab, aes(x = avgBRAR, y = stability)) + 
  ...
  ylim(0, 4) +
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRAR Rel Cov (%)", y = "Soil Moisture Stability", title = "BRAR") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))
...
BRTE_smstab2 <- ggplot(data = BRTE_smstab, aes(x = avgBRTE, y = stability)) + 
  ...
  ylim(0, 4) +
  #geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "BRTE Rel Cov (%)", y = "Soil Moisture Stability", title = "BRTE") +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16))

BRAR_smstab2 / BRTE_smstab2
